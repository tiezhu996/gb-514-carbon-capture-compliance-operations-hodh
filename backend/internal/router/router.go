...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	retirementCoordinator := repository.NewRetirementCoordinator(db)
	deviceLocker := service.NewDeviceLocker()
	...
	complianceDecisionService := service.NewComplianceDecisionService(complianceDecisionRepository, retirementCoordinator, securityService, deviceLocker)
	ruleRetirementService := service.NewRuleRetirementService(permitRuleRepository, retirementCoordinator, securityService, deviceLocker)
	captureUnitHandler := handler.NewCaptureUnitHandler(captureUnitService)
	permitRuleHandler := handler.NewPermitRuleHandler(permitRuleService, ruleRetirementService)
